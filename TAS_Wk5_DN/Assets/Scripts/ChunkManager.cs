...
public class ChunkManager : MonoBehaviour
{
    ...
    public int ChunksSquared = 20; //How many chunks to make, squared
    ...
    void Start()
    {
       //Chunks = new List<Chunk>();
        Chunks = new Chunk[ChunksSquared, ChunksSquared];

        //Make a new GameObject to hold all our chunks in
        GameObject chunkParent = new GameObject("Chunks");
        chunkParent.transform.parent = View;
        
        for (int j = 0; j < ChunksSquared; j++)
        {
            for (int k = 0; k < ChunksSquared; k++)
            {
                ...
                Chunks[j,k] = Instantiate(Resources.Load<GameObject>("Chunk"), origin, Quaternion.identity, chunkParent.transform).GetComponent<Chunk>();
            }
        }
